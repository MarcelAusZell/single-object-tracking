...
def pixel_classification_for_frame(frame, mean, variance, T):
  detections = np.abs(frame - mean) > T * np.sqrt(np.maximum(variance, 10.0))
  return np.uint8(detections) * 255

def single_gaussian_emaf_videos(input_video_path, folder, output_name):
  ...
  mean = first_N_frames.mean(axis=0).astype(np.float32)
  variance = first_N_frames.var(axis=0).astype(np.float32)
  ...
  for _ in tqdm(range(frame_count)):
    ...
    frame_intensities = cv2.cvtColor(current_frame, cv2.COLOR_BGR2GRAY).astype(np.float32)


    detections = pixel_classification_for_frame(frame_intensities, mean, variance, threshold)
    mean = (1 - alpha) * mean  + alpha * frame_intensities
    variance = (1 - alpha) * variance + alpha * ((frame_intensities - mean)**2)

    stacked = np.hstack((np.uint8(mean), np.uint8(np.sqrt(variance)), detections))
